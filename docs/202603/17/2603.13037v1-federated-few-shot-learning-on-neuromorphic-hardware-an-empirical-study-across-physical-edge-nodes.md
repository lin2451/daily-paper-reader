...
title_zh: 类脑硬件上的联邦小样本学习：跨物理边缘节点的实证研究
...
tldr: 本研究探讨了在神经形态硬件上实现联邦少样本学习的挑战，重点解决芯片上脉冲时间依赖可塑性（STDP）产生的二进制权重更新与传统算法不兼容的问题。通过在 BrainChip Akida 处理器上构建双节点实验系统，对比了四种权重交换策略，发现神经元级拼接（FedUnion）能有效保持准确率，而传统的 FedAvg 则表现不佳。研究揭示了特征维度和原型互补性在跨节点知识迁移中的核心作用，为边缘神经形态设备的联邦学习提供了实证指导。
selection_source: fresh_fetch
motivation: 神经形态硬件的二进制权重更新机制与传统联邦学习算法假设的浮点梯度不兼容，限制了其在边缘端的协作学习能力。
method: "在物理边缘节点上部署 BrainChip Akida 处理器，通过 1,580 次实验对比了 FedUnion 和 FedAvg 等四种权重交换策略。"
result: "FedUnion 策略在保持准确率方面显著优于 FedAvg，且增加特征维度能将联邦学习准确率提升至 77.0%。"
conclusion: 神经形态联邦学习的成功依赖于神经元原型的互补性，且领域自适应的特征提取器微调是提升性能的关键因素。
---

## 摘要
由于片上脉冲时间依赖可塑性（STDP）产生的是二进制权重更新，而非标准算法所假设的浮点梯度，类脑硬件上的联邦学习仍处于未探索状态。我们使用 BrainChip Akida AKD1000 处理器构建了一个双节点联邦系统，并在七个分析阶段中进行了约 1,580 次实验试验。在测试的四种权重交换策略中，神经元级拼接（FedUnion）始终能保持准确率，而逐元素权重平均（FedAvg）则会破坏准确率（p = 0.002）。上游特征提取器的领域自适应微调贡献了大部分准确率提升，证实了特征质量是主导因素。将特征维度从 64 扩展到 256，使得最佳策略的联邦准确率达到 77.0%（n=30, p < 0.001）。两个独立的不对称性（更宽的特征对联邦学习的帮助大于对个体学习的帮助，而二值化对联邦学习的损害更大）指向了一种共同的原型互补机制：跨节点迁移的规模随神经元原型的独特性而变化。
