...
## 摘要
在边缘设备上微调大模型受到联邦学习和拆分学习等标准框架中高内存消耗的反向传播（BP）的严重阻碍。虽然用零阶优化替代 BP 可以显著减少内存占用，但通常会导致收敛速度大幅下降。为了解决这一困境，我们提出了混合阶拆分联邦学习（HO-SFL）。通过在拉格朗日框架内重新表述拆分学习过程，HO-SFL 解耦了优化景观：服务器执行精确的一阶更新（即 BP），而客户端进行内存高效的零阶优化。这种混合设计不仅消除了客户端对 BP 的需求，还实现了维度无关的模型聚合，从而大幅降低了通信成本。至关重要的是，我们提供了理论收敛性分析，证明 HO-SFL 缓解了零阶优化中依赖于维度的收敛减速问题，实现了与一阶方法相当的收敛速度。在视觉和语言模态任务上的广泛实验验证了 HO-SFL 在显著降低通信成本和客户端内存占用的同时，实现了与一阶基准相当的收敛速度。
