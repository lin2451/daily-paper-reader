...
method: 提出FedCEF算法，采用解耦近端更新、误差反馈机制及控制变量，并设计了高效的预近端下行策略以减少通信开销。
result: "理论上证明了非凸条件下的亚线性收敛性，实验表明在1%的极低压缩率下仍能保持模型精度并大幅减少总通信量。"
conclusion: FedCEF为处理具有结构化约束的异构联邦学习任务提供了一个通信高效且鲁棒的优化框架。
---

## 摘要
联邦复合优化（FCO）已成为在分布式边缘网络中训练具有结构约束（如稀疏性）模型的极具前景的框架。然而，同时实现通信效率和收敛鲁棒性仍然是一个重大挑战，特别是在处理非光滑正则项、统计异构性以及有偏压缩限制时。为解决这些问题，我们提出了 FedCEF（联邦复合误差反馈），这是一种专为非凸 FCO 量身定制的新型算法。FedCEF 引入了一种解耦的近端更新方案，将近端算子与通信分离，使客户端能够在传输压缩信息的同时在本地处理非光滑项。为了减轻激进量化带来的噪声和非独立同分布（non-IID）数据带来的偏差，FedCEF 将严格的误差反馈机制与控制变量相结合。此外，我们设计了一种通信高效的预近端下行策略，允许客户端在无需显式传输的情况下精确重构全局控制变量。我们在理论上证明了 FedCEF 在一般非凸条件下实现了对有界剩余误差的次线性收敛，且该误差可通过步长和批量大小进行控制。在真实数据集上的广泛实验验证了 FedCEF 即使在极端压缩率（如 1%）下也能保持具有竞争力的模型精度，与未压缩的基准方案相比，显著减少了总通信量。
